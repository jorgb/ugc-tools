...
| `hold` (1-100 %, 100 = whole sample) | `layersv[0].sliceInfo.End` (and `loopEnd`) | SP404 HOLD is the share of the sample that plays (Roland reference manual: at 50 it plays the first half), so the region ends there: `End = start + round(frames * hold / 100) - 1`. Exact |
| `attack` (0-127, 127 = 3 s) | `synthSection.ampEnvelope.Attack.value0` | `attack / 127`, same place on the dial (MPC stores its own as `k/127`; its default is `2/127`). **The seconds are not converted**: the MPC's time law is unverified. Left alone when 0 |
| `release` (0-127, 127 = 3 s), GATE off | `synthSection.ampEnvelope.Decay.value0` | `release / 127`. The SP404 fades out at the end of the hold range; MPC's envelope has `DecayFromEnd` on by default. Unverified |
| `release` (0-127, 127 = 3 s), GATE on | `ampEnvelope.Release.value0`, `AD` and `OneShot` set to `False` | `release / 127`. With GATE on the SP404 fades out when the pad is released; that needs an envelope that responds to note-off, which the default AD one-shot envelope is assumed not to. Unverified |
